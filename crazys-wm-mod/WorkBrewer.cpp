...
double cJobManager::JP_Brewer(sGirl* girl, bool estimate)// not used
{
	...
	if (g_Girls.HasTrait(girl, "Farmers Daughter"))	jobperformance += 10;
	if (g_Girls.HasTrait(girl, "Country Gal"))		jobperformance += 10;
	if (g_Girls.HasTrait(girl, "Mixologist"))		jobperformance += 50;
	...
	if (g_Girls.HasTrait(girl, "Alcoholic"))			jobperformance -= 50;
