...
bool cJobManager::WorkBarSinger(sGirl* girl, sBrothel* brothel, bool Day0Night1, string& summary)
{
	...
	{ss << "She must have the voice of an angel, the customers go on and on about her and always come to listen when she sings.\n\n";wages += 155;
		if (roll <= 20)		{ss << girlName << "'s voice brought many patrons to tears as she sang a song full of sadness.\n";}
		else if (roll <= 40)	{ss << "Wanting to have some fun she encouraged some listeners to sing-along with her.\n";happy += 10;}
		else if (roll <= 60)	{ss << "Feeling a little blue she only sang sad ballads today. You could swear that some customers were crying from the emotion.\n";}
		else if (roll <= 80)	{ss << "The whole room was quiet when " << girlName << " was singing. After she finished, she gathered listeners applauded for minutes.\n";fame += 5;}
		else			{ss << girlName << "'s soothing voice seems to glide over the noise and bustling of the bar.\n";}
	}
	else if (jobperformance >= 185)
	{ss << "She's unbelievable at this and is always getting praised by the customers for her voice.\n\n";wages += 95;
		if (roll <= 25)		{ss << girlName << " begun to acquire her own following - a small crowd of people came in just to listen to her and buy drinks\n";fame += 10; wages += 10;}
		else if (roll <= 50)	{ss << "Her performance was really great, giving the listeners a pleasant time.\n";happy += 5;}
		else if (roll <= 75)	{ss << "When " << girlName << " got on stage the crowd went wild. She didn't disappoint her fans giving one of the best performances in her life.\n";happy += 5;}
		else			{ss << girlName << "'s soothing voice seems to glide over the noise and bustling of the bar.\n";}
	}
	else if (jobperformance >= 145)
	{ss << "Her voice is really good and gets praised by the customers often.\n\n";wages += 55;
		if (roll <= 20)		{ss << "Her singing was pleasing, if bland.  Her voice was nice, if slightly untrained.\n";}
		else if (roll <= 40)	{ss << "She sang every part of the song clearly. " << girlName << " is a really good singer.\n";}
		else if (roll <= 60)	{ss << "This wasn't the best performance of her life time, but in general she did well.\n";}
		else if (roll <= 80)	{ss << "She is good at this. With some work she could be a star.\n";fame += 5;}
		else			{ss << "The slow song " << girlName << " sang at the end of shift really had her full emotion and heart.\n";}
	}
	else if (jobperformance >= 100)
	{ss << "She hits a few right notes but she still has room to improve.\n\n";wages += 15;
		if (roll <= 25)		{ss << "While she won't win any contests, " << girlName << " isn't a terrible singer.\n";}
		else if (roll <= 50)	{ss << "She didn't sing every part clearly but overall she was good.\n";}
		else if (roll <= 75)	{ss << "Maybe she isn't the best but at least she doesn't scare away the customers.\n";}
		else			{ss << "The slow song " << girlName << " sang at the end of shift really had her full emotion and heart.  A pity she was bored and tired.\n";}
	}
	else if (jobperformance >= 70)
	{ss << "She almost never hits a right note. Lucky for you most of your customers are drunks.\n\n";wages -= 5;
		if (roll <= 20)		{ss << "Her singing is barely acceptable, but fortunately the bustling of the bar drowns " << girlName << " out for the most part.\n";}
		else if (roll <= 40)	{ss << "After hearing today's performance, you order your guards to gag her for a week.\n";}
		else if (roll <= 60)	{ss << "Some customers left immediately after " << girlName << " started to sing.\n";wages -= 10;}
		else if (roll <= 80)	{ss << girlName << " singing was awful. Not a single line was sung clearly.\n";happy -= 5;}
		else			{ss << girlName << "'s voice is all over the place as she sings.\n";}
	}
	else
	{ss << "Her voice sounds like nails on a chalk board.  She could be the worst singer ever.\n\n";wages -= 15;
		if (roll <= 14)		{ss << "Her audience seems paralyzed, as if they couldn't believe that a human body could produce those sounds, much less call them \"singing\".\n";happy -= 5;}
		else if (roll <= 28)	{ss << "It's tragic, " << girlName << " really can't sing.\n";}
		else if (roll <= 42)	{ss << "She is just terrible. You could swear that your singing under the shower is far better.\n";}
		else if (roll <= 56)	{ss << girlName << " was the first to get on stage before today's star performance. Seeing the gathered crowd, she froze up being able to let out a single word.\n";}
		else if (roll <= 70)	{ss << girlName << " singing was awful. Angry listeners throw random objects at her.\n";happy -= 5;}
		else if (roll <= 84)	{ss << "Hearing " << girlName << "'s singing gave you a thought to use it as a new torture method.\n";}
		else			{ss << girlName << " bellowed out a melody that caused the bar to go into a panic clearing it quickly.\n";happy -= 10;}
	}
	...
	else if (roll <= 25)	{ss << "\nShe had a pleasant time working."; work += 3; I_confidence += 1;}
